...
n_list = [40] # number of samples in per mode
x_in = 2 # number of neurons in the input layer, i.e., dimension of data
H = 50 # number of neurons in the hidden layer
x_out = 50 # number of neurons in the output layer
learning_rate = 0.005 # learning rate for L+J and G+J
learning_rate_C = 0.0005 # learning rate for G+C and D+C
N_epoch = 1000 # number of training epochs
K = 10 # number of trails
N = 100 # # number of test sets
N_f = 100.0 # number of test sets (float)
sigma = 0.3 # sigma for L+J and G+J
sigma0_u = 0.002  # sigma_0 for L+J and G+J
...
for n in n_list:
    ...
    N_epoch_C = int(500 * 18 * n / batch_size) # number of epochs for training deep networks for G+C and D+C
    ...
    for kk in range(K):
        ...
        pred, STAT_C2ST, model_C2ST, w_C2ST, b_C2ST = C2ST_NN_fit(S,y,N1,x_in,H,x_out,learning_rate_C,N_epoch_C,batch_size,device,dtype)
        ...
        h_u1, threshold_u1, mmd_value_u1 = TST_MMD_u(model_u1(S), N_per, N1, S, sigma, sigma0_u, alpha, device, dtype, 10**(-10), is_smooth=False)
        ...
        for t in range(N_epoch):
            TEMPa = MMDu(S_m, N1, S_m, sigma, sigma0, 10**(-10), is_smooth=False)
        ...
        sigmaOPT = MatConvert(np.sqrt(np.ones(1) * 4), device, dtype)
        sigmaOPT.requires_grad = True
        sigma0OPT = MatConvert(np.sqrt(np.ones(1) * 0.02), device, dtype)
        sigma0OPT.requires_grad = True
        optimizer_sigma0_1 = torch.optim.Adam([epsilonOPT]+[sigmaOPT]+[sigma0OPT], lr=0.05)
        for t in range(N_epoch):
        ...
        h_adaptive_1, threshold_adaptive_1, mmd_value_adaptive_1 = TST_MMD_adaptive_bandwidth(S_m, N_per, N1, S_m, sigma_1,
                                                                                        sigma0_1, alpha, device, dtype)
        ...
        for k in range(N):
            ...
            S = np.concatenate((s1, s2), axis=0)
            S = MatConvert(S, device, dtype)
            # Run two sample tests on generated data
            # L+J
            h_u, threshold_u, mmd_value_u = TST_MMD_u_linear_kernel(model_u(S), N_per, N1, alpha, device, dtype)
            # G+J
            h_u1, threshold_u1, mmd_value_u1 = TST_MMD_u(model_u1(S), N_per, N1, S, sigma, sigma0_u, alpha, device,
                                                         dtype, 10**(-10), is_smooth=False)
            # G+C
            h_adaptive, threshold_adaptive, mmd_value_adaptive = TST_MMD_adaptive_bandwidth(S_m, N_per, N1, S_m, sigma,
                                                                                            sigma0, alpha, device,
                                                                                            dtype)
